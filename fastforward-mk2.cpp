...
void Inheritance()
{
  ...
  public:
    D1virt() {}
    D1virt(std::string&  t) : test("L-VALUE " + t) {}
    D1virt(std::string&& t) : test(std::move("R-VALUE " + t)) {}
    void print() { std::cout << "derived class D1virt size=" << sizeof(*this) << ", test string-->" << test << "<--\n"; }
    D1virt* clone() const { return new D1virt(*this); } //clone the class 
  };
  ...
  D1virt d1virt_foo(foo); // l-value
  d1virt_foo.print();
  D1virt* clone = d1virt_foo.clone();
  std::cout << "CLONE   "; clone->print(); 

  D1virt d1virt_bar("Hello World!"); // r-value
  d1virt_bar.print();
  clone = d1virt_bar.clone();
  std::cout << "CLONE   "; clone->print(); 
} // Inheritance
