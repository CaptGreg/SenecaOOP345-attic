 ...
 *   is given).
 *
 * parallel version with MPI and overlapping computation with communication.
 *
...
int main(int argc, char *argv[]) {
    ...
    for (step = 0; (step < maxsteps) && (maxdiff >= threshold); ++step) {
        ...
        /* finish exchange of boundary information */
        if (myID != 0)
        ...
        /* compute new values for boundary points and include in convergence */
        {
            int i=1;
            ukp1[i]=uk[i]+ (dt/(dx*dx))*(uk[i+1]-2*uk[i]+uk[i-1]);
            diff = fabs(uk[i] - ukp1[i]);
    ...
    if (myID == 0) {
        printf("MPI program v2, overlapping communication and computation (%d processes):\n", num_procs);
